...
pub trait Chars {
    let static base64: Vec<char> = String.from("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/").toCharArray();

    new Chars() {
    }

    pub static fn base64Encode(sequence: BinarySequence, maxLength: i32, buffer: CharSink) {
        if (sequence == nil) {
            return;
        }

        // Compares and returns the minimum of two values.
        let len: i128 = cmp.min(maxLength, sequence.len());
        let mut pad: i32 = 0;
        for (int i = 0; i < len; i += 3) {

            let mut b: i32 = ((sequence.byteAt(i) & 0xFF) << 16) & 0xFFFFFF;
            if (i + 1 < len) {
    ...
    pub static fn asciiCopyTo(char[] chars, int start, int len, long dest) {
        for (int i = 0; i < len; i++) {
