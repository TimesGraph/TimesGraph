...
inline int64_t binary_search(T *data, V value, int64_t low, int64_t high, int32_t scan_dir)
{
    while (high - low > 65)
    {
        ...
        else
        {
            ...
            return scan_dir == -1 ? scroll_up(data, mid, midVal) : scroll_down(data, mid, high, midVal);
        }
    }
    return scan_dir == -1 ? scan_up(data, value, low, high + 1) : scan_down(data, value, low, high + 1);
}
