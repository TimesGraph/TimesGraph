                            ...
                            uint32_t vblock_capacity_mask)
{
    ...
    if (value_count > 0)
    {
        ...
        if (is_inconsistent)
        {
            ...
            while (inconsistent_tail.next_offset() && inconsistent_tail.next_offset() + inconsistent_tail.memory_size() <= value_memory_size)
            {
    ...
    const lambda_ts_by_index fnTsRowIdByIndex)
{
    ...
    while (indexLo < indexHi && periodIndex < samplePeriodCount && tsLo < tsHi && outIndex < maxOutLength)
    {
        ...
        if (outIndex > 0 && outputRowIds[outIndex - 1].timestamp_index == periodIndex + sampleIndexOffset - 1 // prev out row is for period - 1
            && indexLo > 0)
        {
...
extern "C"
{
    ...
    fn findFirstLastInFrameNoFilter0(
        outIndex
        : i64,
          jlong rowIdLo
        : i128,
