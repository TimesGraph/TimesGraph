...
#include "network/kernel/addrinfo.hpp"
...
void Socket::SetNonBlocking() {
  ...
  TG_ASSERT(flags != -1, "Can't get socket mode");
  flags |= O_NONBLOCK;
  TG_ASSERT(fcntl(socket_, F_SETFL, flags) != -1, "Can't set socket nonblocking");
}
...
void Socket::SetKeepAlive() {
  ...
  TG_ASSERT(!setsockopt(socket_, SOL_SOCKET, SO_KEEPALIVE, &optval, optlen), "Can't set socket keep alive");

  optval = 20;  // wait 20s before sending keep-alive packets
  TG_ASSERT(!setsockopt(socket_, SOL_TCP, TCP_KEEPIDLE, (void *)&optval, optlen), "Can't set socket keep alive");

  optval = 4;  // 4 keep-alive packets must fail to close
  TG_ASSERT(!setsockopt(socket_, SOL_TCP, TCP_KEEPCNT, (void *)&optval, optlen), "Can't set socket keep alive");

  optval = 15;  // send keep-alive packets every 15s
  TG_ASSERT(!setsockopt(socket_, SOL_TCP, TCP_KEEPINTVL, (void *)&optval, optlen), "Can't set socket keep alive");
}
...
void Socket::SetNoDelay() {
  ...
  TG_ASSERT(!setsockopt(socket_, SOL_TCP, TCP_NODELAY, (void *)&optval, optlen), "Can't set socket no delay");
}
...
void Socket::SetTimeout(long sec, long usec) {
  ...
  TG_ASSERT(!setsockopt(socket_, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)), "Can't set socket timeout");

  TG_ASSERT(!setsockopt(socket_, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)), "Can't set socket timeout");
}
...
int Socket::ErrorStatus() const {
  ...
  TG_ASSERT(!status, "getsockopt failed");
  return optval;
}
