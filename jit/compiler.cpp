...
struct Function
{
    ...
    void scalar_tail(const instruction_t *istream, size_t size, bool null_check, const x86::Gp &stop, int unroll_factor = 1)
    {
        ...
        {
            TimesGraph::x86::emit_code(c, istream, size, values, null_check, cols_ptr, vars_ptr, input_index);
    ...
    void avx2_loop(const instruction_t *istream, size_t size, uint32_t step, bool null_check, int unroll_factor = 1)
    {
        ...
        for (int i = 0; i < unroll_factor; ++i)
        {
            TimesGraph::avx2::emit_code(c, istream, size, values, null_check, cols_ptr, vars_ptr, input_index);
            ...
            {
                Ymm compacted = TimesGraph::avx2::compress_register(c, row_ids_reg, mask.ymm());
                c.vmovdqu(ymmword_ptr(rows_ptr, output_index, 3), compacted);
                Gp bits = TimesGraph::avx2::to_bits4(c, mask.ymm());
                c.popcnt(bits, bits);
            ...
            {
                Gp bits = TimesGraph::avx2::to_bits(c, mask.ymm(), step);
                TimesGraph::avx2::unrolled_loop2(c, bits.r64(), rows_ptr, input_index, output_index, rows_id_start_offset,
                                                 step);
            }
...
Java_io_TimesGraph_jit_FiltersCompiler_compileFunction(JNIEnv *e,
                                                       jclass cl,
                                                       jlong filterAddress,
                                                       jlong filterSize,
                                                       jint options,
...
Java_io_TimesGraph_jit_FiltersCompiler_freeFunction(JNIEnv *e, jclass cl, jlong fnAddress)
{
#ifndef __aarch64__
...
JNIEXPORT jlong JNICALL Java_io_TimesGraph_jit_FiltersCompiler_callFunction(JNIEnv *e,
                                                                            jclass cl,
                                                                            jlong fnAddress,
                                                                            jlong colsAddress,
                                                                            jlong colsSize,
